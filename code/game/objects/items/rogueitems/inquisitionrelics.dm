		...
		if(istype(W, /obj/item/roguekey/psydonkey))
			...
			switch(relicchoice)
				...
				if("Daybreak - Silver Whip")
					choice = /obj/item/rogueweapon/whip/antique/psywhip/preblessed
				if("Stigmata - Silver Halberd")
					choice = /obj/item/rogueweapon/halberd/psyhalberd/preblessed
				...
				if("Apocrypha - Silver Greatsword")
					choice = /obj/item/rogueweapon/greatsword/psygsword/preblessed
			...
			to_chat(user, span_info("I have chosen the relic, may the Ten guide my hand."))
	...
	if (pulse >= ticks_to_apply)
		...
		for (var/mob/living/carbon/human/H in hearers(7, owner))
			...
			if (!H.has_stress_event(/datum/stressevent/soulchurner))
				...
				if (!H.has_status_effect(/datum/status_effect/buff/churnerprotection))
...
/obj/item/flashlight/flare/torch/lantern/psycenser/afterattack(atom/movable/A, mob/user, proximity)
	...
	if(ismob(A) && on && (user.used_intent.type == /datum/intent/flail/strike/smash/golgotha) && user.cmode)
		...
		for(var/mob/living/carbon/human/H in view(get_turf(src)))
			if(HAS_TRAIT(H, TRAIT_INQUISITION))//Inquisition get VERY depressed seeing an artifact get turned into an ulapool caber.
	...
	if(ishuman(A) && on && (user.used_intent.type == /datum/intent/bless))
		...
		if(HAS_TRAIT(H, TRAIT_INQUISITION))
			...
			to_chat(user, span_warning("They do not share our conviction."))
